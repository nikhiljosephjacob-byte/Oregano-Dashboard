...
function showUpdateAvailableModal(remoteVersion){
  ...
  overlay.style.cssText="position:fixed;inset:0;background:rgba(27,47,74,.78);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;z-index:99999;padding:20px";
  overlay.innerHTML=`
    <div style="background:#0d1524;border:1px solid #f59e0b80;border-radius:12px;padding:24px 28px;max-width:460px;width:100%;box-shadow:0 20px 60px rgba(27,47,74,.6);animation:fadeInUp .3s ease">
...
function showWhatsNewIfNeeded(){
  ...
  overlay.innerHTML=`
    <div style="background:#0d1524;border:1px solid #22C55E80;border-radius:12px;padding:24px 28px;max-width:540px;width:100%;box-shadow:0 20px 60px rgba(27,47,74,.6);animation:fadeInUp .3s ease">
...
function injectLoadingScreen(){
  ...
  ls.innerHTML=`<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:40px 30px;text-align:center;background:radial-gradient(circle at 50% 20%,#FFFFFF 0%,#0a1322 60%,#070d1c 100%);color:#e2e8f0">
...
function ddHTML(id,label,activeSet,items,type){
  ...
  const menuStyle="display:none;position:absolute;top:100%;left:0;z-index:50;margin-top:4px;background:#0b1220;border:1px solid #1b2f4a;border-radius:8px;padding:4px;max-height:280px;overflow-y:auto;min-width:160px;box-shadow:0 12px 30px rgba(27,47,74,.5)";
...
function renderBrands(){
  ...
  const btnH=BR.map(br=>{const act=selBrand===br.n;return`<button onclick="selBrand='${br.n}';renderBrands()" class="brbtn" style="border-color:${act?br.c:"#1b2f4a"};background:${act?br.c+"22":"#FFFFFF"};color:${act?br.c:"#94a3b8"}">${logoImg(br.n,38)}<span style="font-size:12px;font-weight:700;white-space:nowrap">${br.n}</span></button>`;}).join("");
...
function cpcAreaStrengthCard(priorMonth){
  ...
  const tRows=outlets.map(o=>{
    ...
    const cells=aggs.map(ag=>{
      ...
      if(!pct)return`<td style="padding:6px;text-align:center;color:#cbd5e1;font-size:10.5px">—</td>`;
...
function cpcHistoricalRefCard(){
  ...
  const tRows=BR.map(b=>{
    const cells=aggs.map(ag=>{
      ...
      if(total===0)return`<td style="padding:7px 8px;text-align:right;color:#cbd5e1;font-size:10.5px">none</td>`;
...
function cpcHistCompareView(months){
  ...
  return picker+`<div class="card" style="margin-top:12px">
    <div style="margin-bottom:10px"><div style="font-size:12px;font-weight:800;color:#e2e8f0">Brand × Aggregator — A vs B</div><div style="font-size:10.5px;color:#94a3b8;margin-top:2px">Each side uses its own independent filters above. Row-count column flags when the number of underlying sheet entries differs — useful for catching unintended grouping/splitting.</div></div>
...
function cpcRenderOutletLevelSingle(ag,brand,skipToggle){
  ...
  const body=(()=>{
    ...
    let recCell='<span style="color:#cbd5e1;font-size:10px">—</span>';
    if(isCurrentMonth){
...
function renderCPCOutletDetail(){
  ...
  const histRows=rows.map(r=>{
    ...
    const impCell=imp?`<span style="color:${pctClr(imp.salesChg)};font-weight:700">${fmtPct(imp.salesChg)}</span>`:'<span style="color:#cbd5e1">—</span>';
...
function renderCampCalendar(){
  ...
  for(let d=1;d<=dim;d++){
    ...
    let info='';
    if(s.count>0)info=`<div style="display:flex;align-items:center;gap:5px;margin-top:5px;flex-wrap:wrap"><span style="display:inline-flex;align-items:center;gap:3px;font-size:11px;color:#e2e8f0;font-weight:800;background:rgba(13,21,36,.07);padding:2px 8px;border-radius:8px">${s.count} <span style="color:#64748b;font-weight:500;font-size:9px">live</span></span>${s.newStart>0?`<span style="color:#22C55E;font-size:10px;font-weight:700" title="${s.newStart} starting">▶${s.newStart}</span>`:''}${s.newEnd>0?`<span style="color:#EF4444;font-size:10px;font-weight:700" title="${s.newEnd} ending">◀${s.newEnd}</span>`:''}</div>`;
  ...
  const cards=camps.map(c=>{
    ...
    return `<div onclick="selectCamp(${idx})" style="cursor:pointer;background:linear-gradient(135deg,${accent}0d,rgba(13,21,36,.5));border:1px solid ${accent}33;border-left:3px solid ${accent};border-radius:12px;padding:14px;transition:transform .12s,border-color .12s" onmouseover="this.style.transform='translateY(-2px)';this.style.borderColor='${accent}77'" onmouseout="this.style.transform='none';this.style.borderColor='${accent}33'">
      ...
      <div style="display:flex;align-items:center;gap:6px;margin-top:6px;flex-wrap:wrap">
        <span style="font-size:9px;color:#cbd5e1;background:rgba(13,21,36,.05);padding:1px 7px;border-radius:6px">${offer}</span>
        ${coFundChip}
        ${exactChip}
      </div>
...
function campDetailV2HTML(c,idx){
  ...
  const breakdownBox=`<div class="card"><div class="ct">Contribution Breakdown · ${c.brand} on ${c.aggregator}</div>
    ...
    <div style="overflow-x:auto"><table class="tbl"><thead><tr><th>Metric</th><th style="text-align:right">Campaign (${a.cDays}d)<div style="font-size:9px;color:#64748b;font-weight:500;margin-top:2px">${fmtShort(a.effStart)} – ${fmtShort(a.effEnd)}</div></th><th style="text-align:right">Baseline (${a.bDays}d)<div style="font-size:9px;color:#64748b;font-weight:500;margin-top:2px">${fmtShort(a.bStart)} – ${fmtShort(a.bEnd)}</div></th><th style="text-align:right">Per-day Δ</th></tr></thead><tbody>
      ...
      ${a.coFundedDisc>0?`<tr style="color:#60A5FA"><td>＋ ${c.aggregator}-funded co-pay <span style="font-size:9px;color:#94a3b8;font-weight:400">(platform's share of the discount)</span></td><td style="text-align:right">${fmtAEDx(a.coFundedDisc)}</td><td style="text-align:right;color:#cbd5e1">—</td><td style="text-align:right;color:#94a3b8">—</td></tr>
      <tr style="color:#cbd5e1;font-style:italic;font-size:11px"><td>= Total discount to customer</td><td style="text-align:right">${fmtAEDx(a.campDisc+a.coFundedDisc)}</td><td style="text-align:right;color:#cbd5e1">—</td><td style="text-align:right;color:#94a3b8">—</td></tr>`:''}
...
function cmpOutletCard(dA,dB){
  ...
  const panel=`<div class="cmp-outlet-panel" style="display:none;position:absolute;top:100%;left:0;right:0;z-index:30;margin-top:6px;background:#0b1220;border:1px solid #1b2f4a;border-radius:10px;padding:12px;box-shadow:0 12px 30px rgba(27,47,74,.5)">
