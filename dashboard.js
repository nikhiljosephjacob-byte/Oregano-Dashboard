...
const BUILD_VERSION="2026-06-25-033";
const BUILD_NOTES=[
...
function showUpdateAvailableModal(remoteVersion){
  ...
  overlay.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.78);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;z-index:99999;padding:20px";
  overlay.innerHTML=`
    <div style="background:#FFFFFF;border:1px solid #f59e0b80;border-radius:12px;padding:24px 28px;max-width:460px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.6);animation:fadeInUp .3s ease">
...
function showWhatsNewIfNeeded(){
  ...
  overlay.innerHTML=`
    <div style="background:#FFFFFF;border:1px solid #22C55E80;border-radius:12px;padding:24px 28px;max-width:540px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.6);animation:fadeInUp .3s ease">
...
function injectLoadingScreen(){
  ...
  ls.innerHTML=`<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:40px 30px;text-align:center;background:radial-gradient(circle at 50% 20%,#FFFFFF 0%,#0a1322 60%,#070d1c 100%);color:#1E293B">
    ...
    <div style="position:relative;width:180px;height:180px;margin-bottom:18px">
      ...
      <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center">
        <div id="pie-progress-pct" style="font-size:34px;font-weight:800;color:#1E293B;font-variant-numeric:tabular-nums;line-height:1">0%</div>
...
function ddHTML(id,label,activeSet,items,type){
  ...
  const menuStyle="display:none;position:absolute;top:100%;left:0;z-index:50;margin-top:4px;background:#0b1220;border:1px solid #E2E8F0;border-radius:8px;padding:4px;max-height:280px;overflow-y:auto;min-width:160px;box-shadow:0 12px 30px rgba(0,0,0,.5)";
...
function renderOverview(){
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
    ...
    <button onclick="genBrief()" style="background:none;border:1px solid #E2E8F0;border-radius:4px;color:#64748b;padding:3px 9px;font-size:10px;cursor:pointer">↻</button>
  </div>
...
function renderBrands(){
  ...
  const btnH=BR.map(br=>{const act=selBrand===br.n;return`<button onclick="selBrand='${br.n}';renderBrands()" class="brbtn" style="border-color:${act?br.c:"#E2E8F0"};background:${act?br.c+"22":"#FFFFFF"};color:${act?br.c:"#94a3b8"}">${logoImg(br.n,38)}<span style="font-size:12px;font-weight:700;white-space:nowrap">${br.n}</span></button>`;}).join("");
...
function renderOutlets(){
  ...
  const renderTile=t=>{
    ...
    return `<div onclick="selectOutlet('${t.branch.replace(/'/g,"\\'")}')" style="background:#FFFFFF;border:1px solid #E2E8F0;border-radius:10px;padding:12px;cursor:pointer;transition:all .15s" onmouseover="this.style.borderColor='#f59e0b';this.style.background='#111d2e'" onmouseout="this.style.borderColor='#E2E8F0';this.style.background='#FFFFFF'">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
        <div>
          <div style="display:flex;align-items:center;gap:6px">
            <div style="font-size:13px;font-weight:800;color:#1E293B">${t.branch}</div>
...
function renderPlatforms(){
  ...
  const cards=aggSums.map(a=>`<div class="sm" style="cursor:pointer;border-color:${selPlatform===a.ag?a.clr:"#E2E8F0"};box-shadow:${selPlatform===a.ag?`0 0 0 1px ${a.clr}`:"none"}" onclick="selPlatform='${a.ag}';renderPlatforms()"><div style="display:flex;align-items:center;gap:6px;margin-bottom:5px">${logoImg(a.ag,24)}<span style="font-size:9px;color:${a.clr};font-weight:700;text-transform:uppercase">${a.ag}</span></div><div style="font-size:19px;font-weight:800;font-variant-numeric:tabular-nums">${a.orders.toLocaleString()}</div><div style="font-size:11px;color:#64748b">${fmtAED(a.sales)}</div><div style="font-size:11px;color:${pctClr(a.oc)};font-weight:700;margin-top:2px">${fmtPct(a.oc)}</div></div>`).join("");
...
function cpcShell(body,withCrumb){
  const statusBar=cpcModel?`<div style="display:flex;align-items:center;gap:8px"><div style="width:120px;height:6px;background:rgba(0,0,0,.5);border-radius:3px;overflow:hidden"><div style="height:100%;width:100%;background:linear-gradient(90deg,#22C55E,#86EFAC)"></div></div><span style="font-size:11px;color:#22C55E;font-weight:700">✓ Ready</span></div>`:'';
  let crumb='';
  if(withCrumb){
...
function cpcRenderAggLevel(){
  ...
  const monthBtn=(m,label)=>{
    ...
    return `<button onclick="cpcSetAggMonth('${m}')" style="padding:5px 13px;border-radius:7px;border:1px solid ${isActive?'#f59e0b':'rgba(0,0,0,.6)'};background:${isActive?'rgba(245,158,11,.14)':'transparent'};color:${isActive?'#f59e0b':'#94a3b8'};font-size:11.5px;font-weight:700;cursor:pointer">${label}</button>`;
  };
  ...
  const cards=aggs.map(A=>{
    ...
    return `<div onclick="cpcGoBrands('${A.name}')" style="cursor:pointer;background:linear-gradient(135deg,${clr}0d,rgba(255,255,255,.4));border:1px solid ${clr}33;border-radius:14px;padding:16px;position:relative;overflow:hidden;transition:transform .15s,border-color .15s" onmouseover="this.style.borderColor='${clr}88';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='${clr}33';this.style.transform='none'">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
        <div style="display:flex;align-items:center;gap:8px"><div style="width:34px;height:34px;border-radius:9px;background:${clr}22;border:1px solid ${clr}44;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;color:${clr}">${A.name[0]}</div><div><div style="font-size:15px;font-weight:800;color:#1E293B">${A.name}</div><div style="font-size:9px;color:#64748b">${adTypes}</div></div></div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        <div><div style="font-size:9px;color:#64748b;text-transform:uppercase;letter-spacing:.7px">Invested</div><div style="font-size:17px;font-weight:800;color:#1E293B">${fmtAED(inv)}</div></div>
        <div><div style="font-size:9px;color:#64748b;text-transform:uppercase;letter-spacing:.7px">Consumed</div><div style="font-size:17px;font-weight:800;color:#1E293B">${fmtAED(spent)}</div></div>
      </div>
      <div style="width:100%;height:6px;background:rgba(0,0,0,.4);border-radius:3px;overflow:hidden;margin-bottom:10px"><div style="height:100%;width:${Math.min(100,consum)}%;background:linear-gradient(90deg,${clr},${clr}88)"></div></div>
      ...
      ${isViewingCurrent?(()=>{const ct=cpcModel.contractual&&cpcModel.contractual[A.name];if(!ct)return '';const gap=ct.expected-ct.investedSoFar;const metClr=ct.investedSoFar>=ct.expected?'#22C55E':'#FBBF24';return `<div style="margin-top:10px;padding-top:10px;border-top:1px solid rgba(0,0,0,.5)"><div style="font-size:9px;color:#64748b;text-transform:uppercase;letter-spacing:.6px;margin-bottom:3px">Contractual (${(ct.pct*100).toFixed(0)}% of ${cpcMonthLabel(ct.priorMonth)} group sales)</div><div style="display:flex;justify-content:space-between;align-items:baseline"><div><span style="font-size:13px;font-weight:800;color:#1E293B">${fmtAED(ct.investedSoFar)}</span><span style="font-size:10px;color:#64748b"> / ${fmtAED(ct.expected)}</span></div><div style="font-size:10px;font-weight:700;color:${metClr}">${ct.investedSoFar>=ct.expected?'✓ met':fmtAED(gap)+' short'}</div></div></div>`;})():''}
    </div>`;
  }).join('');
...
function cpcRenderBrandLevel(ag){
  ...
  const pastMonths=[1,2,3].map(n=>cpcShiftMonth(cpcModel.curMonth,-n));
  const monthBtn=(m,label)=>{const isActive=targetMonth===m;return `<button onclick="cpcSetAggMonth('${m}')" style="padding:5px 13px;border-radius:7px;border:1px solid ${isActive?'#f59e0b':'rgba(0,0,0,.6)'};background:${isActive?'rgba(245,158,11,.14)':'transparent'};color:${isActive?'#f59e0b':'#94a3b8'};font-size:11.5px;font-weight:700;cursor:pointer">${label}</button>`;};
  const quickViewBar=`<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:14px"><span style="font-size:10px;color:#64748b;font-weight:700;text-transform:uppercase;letter-spacing:.5px">Quick View</span>${monthBtn(cpcModel.curMonth,"This Month")}${pastMonths.map(m=>monthBtn(m,cpcMonthLabel(m))).join("")}</div>`;
  // Ad-type toggle (only if this aggregator has more than one)
  const adTypes=[...A.adTypes];
  const adToggle=adTypes.length>1?`<div style="display:flex;gap:6px;margin-bottom:14px;align-items:center"><span style="font-size:10px;color:#64748b;font-weight:700;text-transform:uppercase">Ad Type</span>${['all',...adTypes].map(t=>`<button onclick="cpcSetAdType('${t}')" style="padding:4px 12px;border-radius:6px;border:1px solid ${cpcAdTypeFilter===t?clr:'rgba(0,0,0,.6)'};background:${cpcAdTypeFilter===t?clr+'22':'transparent'};color:${cpcAdTypeFilter===t?clr:'#94a3b8'};font-size:11px;font-weight:600;cursor:pointer">${t==='all'?'All':t}</button>`).join('')}</div>`:'';
  ...
  const cards=brands.map(B=>{
    ...
    return `<div onclick="cpcGoOutlets('${ag}','${B.name}')" style="cursor:pointer;background:linear-gradient(135deg,${bClr}0d,rgba(255,255,255,.4));border:1px solid ${bClr}33;border-radius:14px;padding:16px;position:relative;overflow:hidden;transition:transform .15s,border-color .15s" onmouseover="this.style.borderColor='${bClr}88';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='${bClr}33';this.style.transform='none'">
      ...
      <div style="font-size:9px;color:${hasData?'#f59e0b':'#94a3b8'};font-weight:700;text-transform:uppercase;letter-spacing:.7px;margin-bottom:8px">${statusLine}</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        <div><div style="font-size:9px;color:#64748b;text-transform:uppercase;letter-spacing:.7px">Budget</div><div style="font-size:16px;font-weight:800;color:#1E293B">${fmtAED(inv)}</div></div>
        <div><div style="font-size:9px;color:#64748b;text-transform:uppercase;letter-spacing:.7px">Consumed</div><div style="font-size:16px;font-weight:800;color:#1E293B">${fmtAED(spent)}</div></div>
      </div>
      <div style="width:100%;height:6px;background:rgba(0,0,0,.4);border-radius:3px;overflow:hidden;margin-bottom:10px"><div style="height:100%;width:${Math.min(100,consum)}%;background:linear-gradient(90deg,${bClr},${bClr}88)"></div></div>
      <div style="display:flex;justify-content:space-between;align-items:center">
...
function cpcTalabatAllocCard(priorMonth){
  ...
  const tRows=rows.map(r=>{
    ...
    return`<tr style="border-bottom:1px solid #E2E8F0"><td style="padding:7px 6px;color:${BMAP[r.brand]?.c||'#fff'};font-weight:700;font-size:11.5px">${r.brand}</td><td style="padding:7px 6px;color:#1E293B;font-size:11.5px">${r.outlet}</td><td style="padding:7px 6px;font-size:11px">${cpcRoasTxt}</td><td style="padding:7px 6px;font-size:11px">${kwRoasTxt}</td><td style="padding:7px 6px;font-size:10.5px;color:#94a3b8">(BE ${r.beVal.toFixed(2)})</td><td style="padding:7px 6px"><span style="color:${r.leverColor};font-weight:700;font-size:10.5px">${r.lever}</span></td><td style="padding:7px 6px;text-align:right">${recTxt}</td></tr>`;
  }).join("");
  ...
  if(ag==="Careem"||ag==="Noon"){
    const outletCards=rows.filter(r=>r.hasAnyHistory||r.bGMV>0).map(r=>{
      ...
      const outletRowsHtml=outletRows.map(o=>{
        ...
        return`<tr style="border-bottom:1px solid rgba(0,0,0,.4)"><td style="padding:5px 8px 5px 20px;color:#475569;font-size:11px">${o.outlet}</td><td style="padding:5px 8px;text-align:right;font-size:10.5px">${roasTxt}</td><td style="padding:5px 8px;text-align:right;color:#94a3b8;font-size:10.5px">${fmtAED(o.oSpent)}</td><td style="padding:5px 8px;text-align:right;color:#94a3b8;font-size:10.5px">${fmtAED(o.oSales)}</td><td style="padding:5px 8px">${tag}</td></tr>`;
      }).join("");
      return`<div style="margin-bottom:8px;border:1px solid rgba(0,0,0,.5);border-radius:6px;overflow:hidden">
...
function cpcDecliningOutletsCard(){
  ...
  const tRows=declining.slice(0,20).map(d=>{
    ...
    return`<tr style="border-bottom:1px solid #E2E8F0"><td style="padding:7px 6px;color:${BMAP[d.brand]?.c||'#fff'};font-weight:700;font-size:11.5px">${d.brand}</td><td style="padding:7px 6px;color:#1E293B;font-size:11.5px">${d.outlet}</td><td style="padding:7px 6px;text-align:right;color:#94a3b8">${fmtAED(d.prior)}</td><td style="padding:7px 6px;text-align:right;color:#475569">${fmtAED(d.cur)}</td><td style="padding:7px 6px;text-align:right;color:#EF4444;font-weight:800">${d.pct.toFixed(0)}%</td><td style="padding:7px 6px;font-size:11px">${stack}</td><td style="padding:7px 6px;color:#475569;font-size:11px">${suggestion}</td></tr>`;
  }).join("");
...
function cpcAreaStrengthCard(priorMonth){
  ...
  const tRows=outlets.map(o=>{
    ...
    return`<tr style="border-bottom:1px solid #E2E8F0"><td style="padding:6px 8px;color:#1E293B;font-weight:600;font-size:11.5px">${o}</td>${cells}</tr>`;
  }).join("");
...
function cpcHistTableView(){
  ...
  const summaryStrip=`<div class="card" style="padding:10px 14px;display:flex;gap:22px;flex-wrap:wrap;align-items:center">
    <div><div style="font-size:9.5px;color:#64748b;text-transform:uppercase">Rows</div><div style="font-size:15px;font-weight:800;color:#1E293B">${rows.length}</div></div>
    <div><div style="font-size:9.5px;color:#64748b;text-transform:uppercase">Budget Allocated</div><div style="font-size:15px;font-weight:800;color:#fbbf24">${fmtAED(totalAlloc)}</div></div>
    <div><div style="font-size:9.5px;color:#64748b;text-transform:uppercase">Budget Spent</div><div style="font-size:15px;font-weight:800;color:#475569">${fmtAED(totalSpent)}</div></div>
    <div><div style="font-size:9.5px;color:#64748b;text-transform:uppercase">Sales</div><div style="font-size:15px;font-weight:800;color:#22C55E">${fmtAED(totalSales)}</div></div>
    <div><div style="font-size:9.5px;color:#64748b;text-transform:uppercase">Orders</div><div style="font-size:15px;font-weight:800;color:#1E293B">${totalOrders.toLocaleString()}</div></div>
    <div><div style="font-size:9.5px;color:#64748b;text-transform:uppercase">Blended ROAS</div><div style="font-size:15px;font-weight:800;color:${totalROAS!=null?'#86EFAC':'#64748b'}">${totalROAS!=null?totalROAS.toFixed(2)+"×":"—"}</div></div>
  </div>`;
  const tRows=rows.map(r=>{
    ...
    return`<tr style="border-bottom:1px solid #E2E8F0"><td style="padding:6px 8px;color:${BMAP[r.brand]?.c||'#fff'};font-weight:700;font-size:11px">${r.brand}</td><td style="padding:6px 8px;color:#1E293B;font-size:11px">${r.branch||"—"}</td><td style="padding:6px 8px;color:${AC[r.aggregator]||'#fff'};font-size:11px">${r.aggregator}</td><td style="padding:6px 8px;color:#94a3b8;font-size:10.5px">${r.adType}</td><td style="padding:6px 8px;color:#94a3b8;font-size:10.5px">${r.startDate||"?"} → ${r.endDate||"?"}</td><td style="padding:6px 8px;text-align:right;color:#94a3b8;font-size:11px">${fmtAED(r.budgetAlloc)}</td><td style="padding:6px 8px;text-align:right;color:#475569;font-size:11px">${fmtAED(r.budgetSpent)}</td><td style="padding:6px 8px;text-align:right;color:#22C55E;font-size:11px">${fmtAED(r.sales)}</td><td style="padding:6px 8px;text-align:right">${roasTxt}</td><td style="padding:6px 8px">${poolTag}</td><td style="padding:6px 8px;color:#94a3b8;font-size:10px">${r.updateDate||"—"}${fundedTag}</td></tr>`;
  }).join("");
...
function cpcHistCompareView(months){
  ...
  return picker+`<div class="card" style="margin-top:12px">
    <div style="margin-bottom:10px"><div style="font-size:12px;font-weight:800;color:#1E293B">Brand × Aggregator — A vs B</div><div style="font-size:10.5px;color:#94a3b8;margin-top:2px">Each side uses its own independent filters above. Row-count column flags when the number of underlying sheet entries differs — useful for catching unintended grouping/splitting.</div></div>
...
function cpcRenderOutletLevel(ag,brand){
  ...
  const sections=aggAdTypes.map(t=>{
    ...
    const headline=`<div style="display:flex;align-items:center;gap:8px;margin:18px 0 8px 0"><span style="font-size:14px;font-weight:800;color:${AC[ag]||'#f59e0b'}">📌 ${brand} on ${ag} — ${t} Campaigns</span><div style="flex:1;height:1px;background:rgba(0,0,0,.6)"></div></div>`;
...
function cpcRenderOutletLevelSingle(ag,brand,skipToggle){
  ...
  const monthBtns=monthsAvail.map(m=>{const isCur=m===curMonthStr;const act=selMonth===m;return `<button onclick="cpcSetMonth('${m}')" style="padding:4px 11px;border-radius:6px;border:1px solid ${act?'#f59e0b':'rgba(0,0,0,.6)'};background:${act?'rgba(245,158,11,.12)':'transparent'};color:${act?'#f59e0b':'#94a3b8'};font-size:11px;font-weight:600;cursor:pointer">${cpcMonthLabel(m)}${isCur?' •':''}</button>`;}).join('');
  const adToggle=(!skipToggle&&aggAdTypes.length>1)?`<div style="display:flex;gap:6px;align-items:center"><span style="font-size:10px;color:#64748b;font-weight:700;text-transform:uppercase">Ad Type</span>${['all',...aggAdTypes].map(tp=>`<button onclick="cpcSetAdType('${tp}')" style="padding:4px 12px;border-radius:6px;border:1px solid ${effAdType===tp?'#f59e0b':'rgba(0,0,0,.6)'};background:${effAdType===tp?'rgba(245,158,11,.12)':'transparent'};color:${effAdType===tp?'#f59e0b':'#94a3b8'};font-size:11px;font-weight:600;cursor:pointer">${tp==='all'?'All':tp}</button>`).join('')}</div>`:'';
  ...
  const totalsRow=`<tr style="border-top:2px solid rgba(245,158,11,.4);background:rgba(245,158,11,.04);font-weight:800"><td style="color:#f59e0b">${isAllPooled?"BRAND POOL TOTAL":isMixed?"TOTAL (incl. pool)":"TOTAL"}</td><td></td><td style="text-align:right;color:#1E293B">${tClicks.toLocaleString()}</td><td style="text-align:right;color:#1E293B">${tOrders.toLocaleString()}</td><td style="text-align:right;color:#1E293B">${fmtAEDExact(tSales)}</td><td style="text-align:right;color:#1E293B">${totAov?totAov.toFixed(0):'—'}</td><td style="text-align:right;color:#1E293B">${totCto!=null?totCto.toFixed(1)+'%':'—'}</td><td style="text-align:right;color:#1E293B">${fmtAEDExact(tBudget)}</td><td style="text-align:right;color:#1E293B">${fmtAEDExact(tSpent)}</td><td style="text-align:right;color:#1E293B">${fmtAEDExact(tLeftover)}</td><td style="text-align:right;color:#f59e0b">${totRoas?totRoas.toFixed(2)+'×':'—'}</td><td style="text-align:right;color:#1E293B">${totBid!=null?'AED '+totBid.toFixed(2):'—'}</td><td style="text-align:right;color:#1E293B">${tFtu.toLocaleString()}</td>${showInvestCol?`<td style="text-align:right">${isCurrentMonth&&tInvest>0?`<span style="color:#22C55E;font-weight:800">+${fmtAEDExact(tInvest)}</span>`:''}</td>`:''}</tr>`;
...
function askAIWidgetHTML(){
  return `<div style="background:rgba(96,165,250,.06);border:1px solid rgba(96,165,250,.2);border-radius:8px;padding:12px;margin-top:14px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <input type="text" id="ai-ask-input" placeholder="e.g. Why did Smokeys drop yesterday?" onkeydown="if(event.key==='Enter')runAskAI()" style="flex:1;min-width:240px;background:#F0F2F5;border:1px solid #E2E8F0;border-radius:5px;color:#1E293B;padding:8px 12px;font-size:12px;outline:none">
    ...
    <div id="ai-ask-suggestions" style="display:flex;flex-wrap:wrap;gap:5px;margin-top:8px">
      ${['Why did orders drop today?','Which outlet needs help?','Suggest 3 quick wins'].map(q=>`<button onclick="document.getElementById('ai-ask-input').value='${q}';runAskAI()" style="background:transparent;border:1px solid #E2E8F0;border-radius:12px;color:#94a3b8;font-size:10px;padding:3px 10px;cursor:pointer">${q}</button>`).join('')}
    </div>
    <div id="ai-ask-answer" style="margin-top:10px;font-size:12px;color:#1E293B;line-height:1.6"></div>
  </div>`;
}
...
function campFilterBar(){
  ...
  // Date filter row
  const inp=(id,val,ph)=>`<input type="date" value="${val||''}" id="${id}" onchange="campSetDate('${id==='cf-from'?'from':'to'}',this.value)" style="background:#0b1220;border:1px solid #E2E8F0;border-radius:6px;color:#1E293B;padding:5px 9px;font-size:11px;font-family:inherit;color-scheme:light" title="${ph}">`;
...
function renderCampCalendar(){
  ...
  for(let d=1;d<=dim;d++){
    ...
    let info='';
    if(s.count>0)info=`<div style="display:flex;align-items:center;gap:5px;margin-top:5px;flex-wrap:wrap"><span style="display:inline-flex;align-items:center;gap:3px;font-size:11px;color:#1E293B;font-weight:800;background:rgba(255,255,255,.07);padding:2px 8px;border-radius:8px">${s.count} <span style="color:#64748b;font-weight:500;font-size:9px">live</span></span>${s.newStart>0?`<span style="color:#22C55E;font-size:10px;font-weight:700" title="${s.newStart} starting">▶${s.newStart}</span>`:''}${s.newEnd>0?`<span style="color:#EF4444;font-size:10px;font-weight:700" title="${s.newEnd} ending">◀${s.newEnd}</span>`:''}</div>`;
  ...
  if(selDay){
    ...
    const renderCampLink=c=>{const clr=BMAP[c.brand]?.c||'#888';const addonTag=(c.addons&&c.addons.length)?` <span style="background:rgba(232,214,20,0.15);color:#E8D614;font-size:9px;font-weight:700;padding:1px 6px;border-radius:8px;margin-left:5px">+ ${c.addons.map(a=>a.name).join(', ')}</span>`:'';return `<div onclick="selectCamp(${campaignData.indexOf(c)})" style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:#F8FAFC;border-left:3px solid ${clr};border-radius:5px;cursor:pointer;margin-bottom:5px" onmouseover="this.style.background='#E2E8F0'" onmouseout="this.style.background='#111d2e'">${logoImg(c.brand,22)}<div style="flex:1;min-width:0"><div style="font-size:12px;font-weight:700;color:#1E293B">${c.name}${addonTag}</div><div style="font-size:10px;color:#94a3b8;margin-top:1px"><span style="color:${clr}">${c.brand}</span> · <span style="color:${AC[c.aggregator]||'#888'}">${c.aggregator}</span> · ${c.outlet||'All'} · ${fmtCampDateRange(c.startDate,c.endDate)}</div></div><button style="background:#f59e0b22;border:1px solid #f59e0b44;border-radius:5px;color:#f59e0b;padding:4px 10px;font-size:10px;font-weight:600;cursor:pointer">View →</button></div>`;};
    ...
    return `<div onclick="selectCamp(${idx})" style="cursor:pointer;background:linear-gradient(135deg,${accent}0d,rgba(255,255,255,.5));border:1px solid ${accent}33;border-left:3px solid ${accent};border-radius:12px;padding:14px;transition:transform .12s,border-color .12s" onmouseover="this.style.transform='translateY(-2px)';this.style.borderColor='${accent}77'" onmouseout="this.style.transform='none';this.style.borderColor='${accent}33'">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        <div style="min-width:0;flex:1">
          <div style="font-size:13px;font-weight:800;color:#1E293B;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${c.name||'Campaign'}</div>
...
function campOutletBreakdownHTML(c,a){
  ...
  const rows=branchesInScope.map(br=>{
    ...
    return `<tr><td style="font-weight:700;color:#1E293B">${br}</td><td style="text-align:right;font-weight:700;color:#1E293B;font-variant-numeric:tabular-nums">${Math.round(cs.orders).toLocaleString()}</td><td style="text-align:right;color:#94a3b8;font-variant-numeric:tabular-nums">${fmtAEDx(cs.sales)}</td>${pwCell}${pmCell}${p2Cell}</tr>`;
  }).filter(Boolean).join('');
  if(!rows)return '';
  ...
  const totalsRow=`<tr style="border-top:2px solid rgba(245,158,11,.3);background:rgba(245,158,11,.04)"><td style="font-weight:800;color:#f59e0b">TOTAL / AVG</td><td style="text-align:right;font-weight:800;color:#1E293B;font-variant-numeric:tabular-nums">${Math.round(tClickOrders).toLocaleString()}</td><td style="text-align:right;font-weight:800;color:#1E293B;font-variant-numeric:tabular-nums">${fmtAEDx(tClickSales)}</td><td style="text-align:right;font-variant-numeric:tabular-nums">${totUpliftCell(avgPw)}</td><td style="text-align:right;font-variant-numeric:tabular-nums">${totUpliftCell(avgPm)}</td><td style="text-align:right;font-variant-numeric:tabular-nums">${totUpliftCell(avgP2)}</td></tr>`;
...
function campDetailV2HTML(c,idx){
  ...
  const verdictBox=verdict?`<div style="background:${verdictClr}15;border:1px solid ${verdictClr}40;border-radius:12px;padding:14px 16px;margin-bottom:14px;display:flex;gap:12px;align-items:flex-start"><div style="font-size:20px">${verdictIcon}</div><div style="font-size:13px;color:#1E293B;line-height:1.6;font-weight:500">${verdict}</div></div>`:'';
  ...
  const breakdownBox=`<div class="card"><div class="ct">Contribution Breakdown · ${c.brand} on ${c.aggregator}</div>
    ...
    <div style="overflow-x:auto"><table class="tbl"><thead><tr><th>Metric</th><th style="text-align:right">Campaign (${a.cDays}d)<div style="font-size:9px;color:#64748b;font-weight:500;margin-top:2px">${fmtShort(a.effStart)} – ${fmtShort(a.effEnd)}</div></th><th style="text-align:right">Baseline (${a.bDays}d)<div style="font-size:9px;color:#64748b;font-weight:500;margin-top:2px">${fmtShort(a.bStart)} – ${fmtShort(a.bEnd)}</div></th><th style="text-align:right">Per-day Δ</th></tr></thead><tbody>
      ...
      <tr style="border-top:2px solid rgba(245,158,11,.3);font-weight:800"><td style="color:#f59e0b">Contribution</td><td style="text-align:right;color:#1E293B">${fmtAEDx(a.campContribTotal)}</td><td style="text-align:right;color:#1E293B">${fmtAEDx(a.baseContribTotal)}</td><td style="text-align:right;color:${incrClr}">${a.incrContribPerDay>=0?'+':''}${fmtAEDx(a.incrContribPerDay)}</td></tr>
    </tbody></table></div></div>`;
  let scenarioBox='';
...
function campDetailHTML(c,idx){
  ...
  const prosCons=`<div class="g2" style="margin-bottom:12px"><div class="sm"><div class="ct" style="color:#22C55E">✅ Pros</div>${pros.map(p=>`<div style="display:flex;gap:8px;margin-bottom:7px;font-size:12px;color:#1E293B;line-height:1.5"><span style="color:#22C55E;flex-shrink:0">▸</span><span>${p}</span></div>`).join('')}</div><div class="sm"><div class="ct" style="color:#EF4444">⚠️ Cons</div>${cons.map(p=>`<div style="display:flex;gap:8px;margin-bottom:7px;font-size:12px;color:#1E293B;line-height:1.5"><span style="color:#EF4444;flex-shrink:0">▸</span><span>${p}</span></div>`).join('')}</div></div>`;
...
async function runCampAI(idx){
  ...
  try{
    ...
    content.innerHTML=`<div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:12px"><div style="padding:3px 12px;border-radius:10px;font-size:11px;font-weight:700;background:${vc}22;color:${vc};border:1px solid ${vc}44;white-space:nowrap">${(ai.verdict||'').replace(/_/g,' ')}</div><div style="font-size:13px;color:#1E293B;line-height:1.6">${ai.assessment}</div></div><div class="g2"><div><div style="font-size:10px;color:#f59e0b;font-weight:700;text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px">📋 Suggestions</div>${(ai.suggestions||[]).map((s,i)=>`<div style="display:flex;gap:8px;margin-bottom:8px"><div style="background:#f59e0b;color:#000;font-size:10px;font-weight:700;width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0">${i+1}</div><div style="font-size:12px;color:#1E293B;line-height:1.5">${s}</div></div>`).join('')}</div><div><div style="font-size:10px;color:#22C55E;font-weight:700;text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px">📈 What Works for ${c.brand} on ${c.aggregator}</div><div style="font-size:12px;color:#1E293B;line-height:1.7;background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.15);border-radius:6px;padding:10px">${ai.bestPractice}</div></div></div>`;
...
function bundleDetailHTML(bundle){
  ...
  const isScoped=scopeStr!=="All outlets";
  const scopeBadge=isScoped?`<div style="margin-top:10px;padding:8px 12px;background:rgba(96,165,250,.08);border-left:3px solid #60A5FA;border-radius:4px;display:flex;align-items:center;gap:10px;flex-wrap:wrap"><div style="font-size:18px">📍</div><div style="flex:1;min-width:200px"><div style="font-size:10px;color:#60A5FA;font-weight:700;text-transform:uppercase;letter-spacing:.8px">Location-Scoped Analysis</div><div style="font-size:11px;color:#94a3b8;margin-top:2px;line-height:1.5">${scopeStr} — compared against the <strong style="color:#1E293B">same outlets only</strong> in the prior period.</div></div></div>`:'';
  const header=`<div class="card" style="margin-bottom:12px;border-left:4px solid ${brandClr}"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap">
    <div style="flex:1;min-width:280px"><div style="font-size:9px;color:#FBBF24;font-weight:800;letter-spacing:1.5px;text-transform:uppercase">🎯 Coordinated Campaign Bundle</div><div style="font-size:18px;font-weight:800;color:#1E293B;margin-top:4px">${bundle.brand} on ${bundle.aggregator}</div><div style="font-size:11px;color:#94a3b8;margin-top:4px">${fmtCampDateRange(bundle.startDate,bundle.endDate)} · ${a.days} day${a.days>1?'s':''} · ${bundle.outlet||'All'} outlets · ${bundle.campaigns.length} concurrent segments</div>${scopeBadge}</div>
...
async function renderCampaigns(){
  ...
  try{
    ...
    const tabH=tabs.map(([k,l,n])=>{const act=campTab===k;const cnt=n!=null?` <span style="background:${act?'rgba(245,158,11,.25)':'rgba(100,116,139,.2)'};color:${act?'#FBBF24':'#94a3b8'};font-size:9px;font-weight:800;padding:1px 6px;border-radius:8px;margin-left:3px">${n}</span>`:'';return `<button onclick="campTab='${k}';renderCampaigns()" style="padding:7px 14px;border-radius:7px;border:1px solid ${act?'#f59e0b':'rgba(0,0,0,.6)'};background:${act?'linear-gradient(180deg,rgba(245,158,11,.18),rgba(245,158,11,.08))':'transparent'};color:${act?'#f59e0b':'#94a3b8'};font-size:12px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:3px;transition:all .15s">${l}${cnt}</button>`;}).join('');
    let main='';
...
function renderKPIPlatformGrid(){
  ...
  const tileH=platforms.map(p=>{
    ...
    return `<div onclick="selectKPIPlatform('${p}')" style="background:#FFFFFF;border:1px solid ${bad>0?'#EF444455':'#E2E8F0'};border-radius:10px;padding:14px;cursor:pointer;transition:all .15s" onmouseover="this.style.borderColor='#f59e0b'" onmouseout="this.style.borderColor='${bad>0?'#EF444455':'#E2E8F0'}'">
      ...
      ${okPct!=null?`<div style="margin-top:8px;height:5px;background:#E2E8F0;border-radius:3px;overflow:hidden"><div style="height:100%;width:${okPct}%;background:${okPct>=80?'#22C55E':okPct>=60?'#FBBF24':'#EF4444'}"></div></div><div style="font-size:10px;color:#64748b;margin-top:4px">${okPct}% on target</div>`:`<div style="font-size:11px;color:#64748b;margin-top:8px">No tracked KPIs</div>`}
    </div>`;
  }).join("");
...
function renderKPILaggingPanel(){
  ...
  const rowsH=platforms.filter(p=>byAgg[p].length>0).map(p=>{
    ...
    return `<div style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:1px solid rgba(0,0,0,.4)">
...
function renderKPIGoogleOutlets(){
  ...
  const cards=sorted.map(r=>{
    ...
    return `<div onclick="kpiSelectedOutlet='${r.outlet.replace(/'/g,"\\'")}';kpiSelectedBrand='${r.brand}';kpiSelectedAggregator='Google Maps';kpiSelectedKPIName='${r.kpiName.replace(/'/g,"\\'")}';renderKPI()" style="position:relative;background:#FFFFFF;border:1px solid #E2E8F0;border-left:4px solid ${accent};border-radius:10px;padding:14px 16px;cursor:pointer;min-height:96px;display:flex;flex-direction:column;justify-content:space-between" onmouseover="this.style.borderColor='#f59e0b'" onmouseout="this.style.borderColor='#E2E8F0'">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        <div style="display:flex;align-items:center;gap:8px">${logoImg(r.brand,24)}<div><div style="font-size:14px;font-weight:800;color:#1E293B">${r.brand} ${r.outlet}</div><div style="font-size:10px;color:#64748b">${st.label}</div></div></div>
        ${flag}
      </div>
...
function renderKPIPlatformView(){
    ...
    return `<div style="margin-top:12px">
      ...
      ${list.map((r,i)=>{
        ...
        return `<div style="display:flex;align-items:baseline;font-size:12px;padding:3px 0;gap:6px">
          <span style="color:#1E293B;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;flex-shrink:1" title="${r.outlet}">${r.outlet}</span>
  ...
  const tiles=brandsPresent.map(b=>{
    ...
    return `<div onclick="selectKPIBrand('${b.n}')" style="background:#FFFFFF;border:1px solid ${bad>0?'#EF444455':'#E2E8F0'};border-radius:10px;padding:12px 11px;cursor:pointer" onmouseover="this.style.borderColor='#f59e0b'" onmouseout="this.style.borderColor='${bad>0?'#EF444455':'#E2E8F0'}'">
      ...
      ${worstHTML?`<div style="margin-top:10px;padding-top:10px;border-top:1px solid rgba(0,0,0,.6)">${worstHTML}</div>`:''}
    </div>`;
  }).join("");
  pg.innerHTML=`<div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;flex-wrap:wrap">
      <button onclick="backToKPIPlatforms()" style="background:none;border:1px solid #E2E8F0;border-radius:6px;color:#64748b;padding:6px 12px;cursor:pointer;font-size:12px">← All Platforms</button>
...
function renderKPIMetricView(){
  ...
  const cards=sorted.map(r=>{
    ...
    return `<div onclick="kpiSelectedOutlet='${r.outlet.replace(/'/g,"\\'")}';kpiSelectedAggregator='${r.aggregator}';kpiSelectedKPIName='${m.replace(/'/g,"\\'")}';renderKPI()" style="position:relative;background:#FFFFFF;border:1px solid #E2E8F0;border-left:4px solid ${accent};border-radius:10px;padding:14px 16px;cursor:pointer;min-height:96px;display:flex;flex-direction:column;justify-content:space-between" onmouseover="this.style.borderColor='#f59e0b';this.style.borderLeftColor='${accent}'" onmouseout="this.style.borderColor='#E2E8F0';this.style.borderLeftColor='${accent}'">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        <div><div style="font-size:15px;font-weight:800;color:#1E293B">${r.outlet}</div><div style="font-size:10px;color:#64748b">${st.label}</div></div>
        ${flag}
      </div>
      <div style="display:flex;align-items:flex-end;justify-content:space-between;gap:8px;margin-top:8px">
  ...
  pg.innerHTML=`<div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;flex-wrap:wrap">
      ...
      <div style="display:flex;align-items:center;gap:8px">${logoImg(b,24)}<span style="font-size:16px;font-weight:800;color:${bc}">${b}</span><span style="color:#64748b">·</span><span style="font-size:14px;font-weight:700;color:${clr}">${p}</span><span style="color:#64748b">·</span><span style="font-size:15px;font-weight:800;color:#1E293B">${m}</span></div>
    </div>
    ${missingNote}
...
function cmpPanel(side){
  ...
  return `<div style="flex:1;min-width:300px;background:#FFFFFF;border:1px solid ${accent}55;border-radius:12px;padding:14px">
    ...
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
      <input type="date" value="${cfg.start||""}" data-act="cmpDate" data-v1="${side}" data-v2="start" style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:6px;color:#1E293B;padding:7px 12px;font-size:13px;font-weight:600;color-scheme:light;min-width:135px">
      <span style="color:#64748b">→</span>
      <input type="date" value="${cfg.end||""}" data-act="cmpDate" data-v1="${side}" data-v2="end" style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:6px;color:#1E293B;padding:7px 12px;font-size:13px;font-weight:600;color-scheme:light;min-width:135px">
    </div>
...
function cmpOutletCard(dA,dB){
  ...
  const col=(title,clr,list)=>`<div style="flex:1;min-width:120px"><div style="font-size:9px;font-weight:700;color:${clr};text-transform:uppercase;letter-spacing:.6px;margin-bottom:5px">${title} (${list.length})</div>${list.length?list.map(o=>`<div style="font-size:11px;color:#475569;padding:1px 0">${o}</div>`).join(""):`<div style="font-size:11px;color:#64748b">—</div>`}</div>`;
  // The panel is hidden by default and shown on hover (CSS sibling, inline handlers as fallback)
  const panel=`<div class="cmp-outlet-panel" style="display:none;position:absolute;top:100%;left:0;right:0;z-index:30;margin-top:6px;background:#0b1220;border:1px solid #E2E8F0;border-radius:10px;padding:12px;box-shadow:0 12px 30px rgba(0,0,0,.5)">
...
function renderCompare(){
  ...
  if(cmpExpandedRow){
    ...
    const oRows=branchRows.map(r=>({
      cells:[
        `<strong style="color:#1E293B;font-size:12px">${r.branch}</strong>`,
        `<span style="color:#60A5FA">${r.a.orders.toLocaleString()}</span>`,
        `<span style="color:#F59E0B">${r.b.orders.toLocaleString()}</span>`,
        `<span style="color:${pctClr(r.oDiff)};font-weight:700">${fmtPct(r.oDiff)}</span>`,
        `<span style="color:#60A5FA">${fmtAED(r.a.sales)}</span>`,
  ...
  pg.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:10px">
      <div style="font-size:18px;font-weight:800;color:#1E293B">⚖️ Comparison</div>
...
function cmpDrawChart(dA,dB){
  ...
  ]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:"index",intersect:false},plugins:{legend:{display:true,labels:{color:"#94a3b8",font:{size:10},boxWidth:12,padding:10}},tooltip:{callbacks:{label:c=>`${c.dataset.label.split(" · ")[0]}: ${fmtV(c.raw)}`}}},scales:{x:{ticks:{color:"#64748b",font:{size:9}},grid:{color:"rgba(0,0,0,.08)"},border:{display:false}},y:{ticks:{color:"#64748b",font:{size:9},callback:v=>v>=1000?`${(v/1000).toFixed(0)}K`:v},grid:{color:"rgba(0,0,0,.08)"},border:{display:false}}}}});
}
